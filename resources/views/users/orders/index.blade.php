      ...
      <div class="order-body">
        ...
        <div class="order-info-item">
          ...
          <span class="info-value">
              ${order.total_items > 1 ? `${order.total_items} Products` : `${order.total_items} Product`}
          </span>
        </div>
        <div class="order-info-item">
      ...
      <div class="order-footer">
        <div class="customer-info">
          Customer: <span class="customer-name">${order.buyer}</span>
            ...
            <a href="{{ url('users/order/summary') }}/${order.id}" class="view-details-btn">
            View Details
            </a>
      </div>
    `;
            ...
            // ✅ Filter functionality (fixed and improved)
                ...
                filterLinks.forEach(link => {
                    link.addEventListener('click', function(e) {
                        ...
                        orderCards.forEach(card => {
                            // Normalize filter and status text
                            const cardStatus = card.dataset.status.trim().toLowerCase()
                                .replace(/\s+/g, '');
                            const filterValue = filter.trim().toLowerCase().replace(/\s+/g,
                                '');

                            // Compare and show/hide cards accordingly
                            if (filter === 'all' || cardStatus === filterValue) {
